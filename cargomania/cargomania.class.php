...
/*MANTENIMIENTO DE CLIENTES*/
    function guardar_cliente($campos){
        $con = new DBManager(); //creamos el objeto $con a partir de la clase DBManager
        $dbh = $con->conectar("pgsql"); //Pasamos como parametro que la base de datos a utilizar para el caso MySQL.
        $sql = "INSERT INTO cliente(nombre_empr,giro,num_reg,direccion,nom_represent,apellido_represent,telefono_represent,correo)VALUES(:nomb_empr,:giro,:numero_reg,:direccion,:nombre_repre,:apellido_repre,:telefono_repre,:email_repre)";
        $query = $dbh->prepare($sql); // Preparamos la consulta para dejarla lista para su ejecucion
        $query->bindParam(":codigo",$campos[0]);
...
/*MANTENIMIENTO DE Agente de Bodega*/
    function guardar_agentebodega($campos){
        $con = new DBManager(); //creamos el objeto $con a partir de la clase DBManager
        $dbh = $con->conectar("pgsql"); //Pasamos como parametro que la base de datos a utilizar para el caso MySQL.
        $sql = "INSERT INTO agente_bodega(nombre_aget_bod,direccion_aget_bod,nombre_repre_aget_bod,apellido_repre_aget_bod,pais_aget_bod) VALUES(:nomb_aget,:direccion_aget,:nombre_repre,:apellido_repre,:pais_aget)";
        $query = $dbh->prepare($sql); // Preparamos la consulta para dejarla lista para su ejecucion
        $query->bindParam(":codigo",$campos[0]);
...
/*MANTENIMIENTO DE Servicio de Bodega*/
    ...
    function guardar_empleado($campos){
        $con = new DBManager(); //creamos el objeto $con a partir de la clase DBManager
        $dbh = $con->conectar("pgsql"); //Pasamos como parametro que la base de datos a utilizar para el caso MySQL.
        $sql = "INSERT INTO empleado(nombre_epl,apellido_epl,direccion_epl,telefono_epl,correo_epl,num_seguro,fk_sucursal) VALUES(:nombre_epl,:apellido_epl,:direccion_epl,:telefono_epl,:correo_epl,:num_seguro,:fk_sucursal)";
        $query = $dbh->prepare($sql); // Preparamos la consulta para dejarla lista para su ejecucion
...
/*MANTENIMIENTO DE PROVEEDOR*/
    function guardar_proveedor($campos){
        $con = new DBManager(); //creamos el objeto $con a partir de la clase DBManager
        $dbh = $con->conectar("pgsql"); //Pasamos como parametro que la base de datos a utilizar para el caso MySQL.
        $sql = "INSERT INTO proveedor(nombre_prov,pais_prov,direccion,telefono_prov,correo_prov) VALUES(:nombre,:pais,:direccion,:telefono,:correo)";
        $query = $dbh->prepare($sql); // Preparamos la consulta para dejarla lista para su ejecucion
        $query->bindParam(":codigo",$campos[0]);
 ...
 /*MANTENIMIENTO DE ADUANA_LLEGADA*/
    function guardar_aduana_llegada($campos){
        $con = new DBManager(); //creamos el objeto $con a partir de la clase DBManager
        $dbh = $con->conectar("pgsql"); //Pasamos como parametro que la base de datos a utilizar para el caso MySQL.
        $sql = "INSERT INTO aduana_llegada(nombre_adu_llega,direccion_adu_llega,tipo_adu_llega,pais_adu_llega) VALUES(:nombre,:direccion,:tipo,:pais)";
        $query = $dbh->prepare($sql); // Preparamos la consulta para dejarla lista para su ejecucion
        $query->bindParam(":codigo",$campos[0]);
        $query->bindParam(":nombre",$campos[1]);
